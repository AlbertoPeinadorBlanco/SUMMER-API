...
for (const item of data) {
    ...
    const name = item.accommodation_name.replace(/'/g, "''");
    const location = item.location_name.replace(/'/g, "''");
    
    // Check type based on name
    let type = 'camping';
    if (name.toLowerCase().includes('hotel')) type = 'hotel';
    if (name.toLowerCase().includes('apartamento')) type = 'apartment';
    if (name.toLowerCase().includes('surf house')) type = 'surfhouse';

    const desc = item.brief_description.replace(/'/g, "''");
    // Optionally append difficulty to description
    const fullDesc = `${desc}\\n\\nAccess to Sea Difficulty: ${item.access_to_sea_difficulty.replace(/'/g, "''")}`;

    sql += `INSERT INTO accommodations (name, type, location, image_url, description_en, description_es, link) VALUES ('${name}', '${type}', '${location}', '${item.image_url}', '${fullDesc}', '${fullDesc}', '${link}');\n`;
}

fs.writeFileSync('c:/Users/jazzm/Desktop/SURFING/CODE/deploy/accommodations_seed.sql', sql);
console.log('Saved to deploy/accommodations_seed.sql');
